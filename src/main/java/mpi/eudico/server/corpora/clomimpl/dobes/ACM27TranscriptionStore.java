...
public class ACM27TranscriptionStore implements TranscriptionStore, ServerLogger {
    ...
    public void loadTranscription(Transcription theTranscription, DecoderInfo decoderInfo) {
        ...
        ArrayList linkedFileDescriptors = parser.getLinkedFileDescriptors(trPathName);
 ...
 //           typesCopy.add(i, linguisticTypes.get(i));
        ...
        while (tierIter.hasNext()) {
            ...
            while (it1.hasNext()) {
        		...
        		if(annotationRecord.getCvEntryId() != null && !annotationRecord.getCvEntryId().equals("")) {
                ...
                if (extReferences != null && annotationRecord.getExtRefId() != null) {
                	...
                	for (int i = 0; i < extRefIds.length; i++) {
						...
						if (eri != null && (eri.getReferenceType() == ExternalReference.ISO12620_DC_ID 
							...
							try {
								((AbstractAnnotation) annotation).addExtRef((ExternalReferenceImpl) eri.clone());
        ...
        // add lastUsedAnnotationIdProperty
        ...
        if ((cvTable != null) && (cvTable.size() > 0)) {
        	...
        	while (cvIt.hasNext()) {
        		...
        		CVRecord contents = (CVRecord)cvTable.get(cvName);
        		...
        		if (contents.hasContents()) { 
        			...
        			if(! (cv instanceof ExternalCV)) {
        				Object next;
        				CVEntry entry;
        				ArrayList entriesInRecord = contents.getEntries();
        				for (int i = 0; i < entriesInRecord.size(); i++) {
        					next = entriesInRecord.get(i);
        					if (next instanceof CVEntryRecord) {
        						if (((CVEntryRecord) next).getId() != null) {
        							entry = new ExternalCVEntry(
        									((CVEntryRecord) next).getValue(),
        									((CVEntryRecord) next)
        											.getDescription(),
        									((CVEntryRecord) next).getId());
        						} else {
        							entry = new CVEntry(((CVEntryRecord) next)
        									.getValue(), ((CVEntryRecord) next)
        									.getDescription());
        						}
        						if (entry != null) {
        							...
        							if (extReferences != null
        									&& ((CVEntryRecord) next)
        											.getExtRefId() != null) {
        								ExternalReferenceImpl eri = (ExternalReferenceImpl) extReferences
        										.get(((CVEntryRecord) next)
        												.getExtRefId());
        								if (eri != null) {
        									try {
        										entry.setExternalRef(eri
        												.clone());
        									} catch (CloneNotSupportedException cnse) {
		...
		HashMap lexSrvcTable = parser.getLexiconServices(trPathName);
        if ((lexSrvcTable != null) && (lexSrvcTable.size() > 0)) {
        	LexiconQueryBundle2 bundle = null;
        	Iterator lexIt = lexSrvcTable.keySet().iterator();
        	while (lexIt.hasNext()) {
        		String lexRef = (String)lexIt.next();
        		if (lexRef == null) {
        			continue;
        		}
        		LexiconServiceRecord contents = (LexiconServiceRecord)lexSrvcTable.get(lexRef);
