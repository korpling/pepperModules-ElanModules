...
public class EAF20 {
	...
	*/
	...
	public final Element newAnnotationDocument
		...
		result.setAttribute("xsi:noNamespaceSchemaLocation", "http://www.mpi.nl/tools/elan/EAFv2.0.xsd");
		result.setAttribute("DATE",   creationDate);
		result.setAttribute("AUTHOR", author);
		result.setAttribute("VERSION", version);
		result.setAttribute("FORMAT", "2.0");
		return result;
    }
	/**
	...
	public final Element newHeader (String mediaFile) {
		if (mediaFile == null) throw new RuntimeException("EAF");
		return newHeader(mediaFile, EAF20.TIME_UNIT_MILLISEC);
    }
	/**
	   Use result in annotationDocument.appendChild();
	...
	/**
	   ...
	   @param id   --->documentation missing<---
	 */
	public final Element newLinguisticType (String id, boolean timeAlignable, boolean graphicReferences, String constraint){
		if (id == null) throw new RuntimeException("EAF");
	...
	*/
	...
	/**
	   Testing the class.
	 */
    public EAF20(boolean test) throws Exception {
		this();

		//test...
		Element e = newAnnotationDocument(new Date()+"", "gregor", "version 2.0");
		appendChild(e);
