    ...
    */
    ...
    public Vector getTimeSlotsForNewAnnotation(long begin, long end,
        ...
        if (insertAtParentEnd) {
            ...
            currEndAnn = (AlignableAnnotation) ((TierImpl) forTier).getAnnotationAtTime(end - 1,
                    true);
        } else {
            currEndAnn = (AlignableAnnotation) ((TierImpl) forTier).getAnnotationAtTime(end,
                    true);
            ...
            if (!multiOverlappingAnns) {
                ...
                if (!insertAtParentEnd) {
                    ...
                    if (!oldBeginTS.isTimeAligned()) {
                        AlignableAnnotation naa = getFirstAlignedAnnotation((TierImpl) forTier, currEndAnn);
                        if (naa != null) {
                            currEndAnn = naa;
                            oldBeginTS = currEndAnn.getBegin();
                        }
    ...
    public void detachAnnotation(Annotation theAnn, Tier theTier) {
        ...
        if (enclosedAnnots.size() > 0) {
            ...
            if (prev != null) {
                ...
                Vector endingAtTs = ((TranscriptionImpl) (theTier.getParent())).getAnnotsEndingAtTimeSlot(prev.getEnd(), theTier, true);
                ...
                Vector beginningAtTs = ((TranscriptionImpl) (theTier.getParent())).getAnnotsBeginningAtTimeSlot(next.getBegin(), theTier, true);
    ...
    private void setAlignableBeginSlot(TierImpl nextT, AlignableAnnotation parentAA, TimeSlot ets, long end) {
        ...
        Vector childT = nextT.getChildTiers();
        TierImpl ch;
        if (nextAA != null) {
