...
public class AlignableAnnotation extends AbstractAnnotation {
    ...
    public void updateTimeInterval(long beginTime, long endTime) {
        ...
        if (this.hasParentAnnotation()) {
            ...
            if (beginTime <= parent.getBegin().getTime()) {
                ...
                // forced out of the interval and later detached from the tier (see alsp TierImpl)
        ...
        (((TranscriptionImpl) ((TierImpl) getTier()).getParent())).getConnectedAnnots(connectedAnnots,
        ...
        if (!((getBegin() == graphEndpoints[0]) && hasParentAnnotation()) &&
                !(parent != null && getBegin() == parent.getBegin())) {
            getBegin().setTime(beginTime);
        }

        if (!((getEnd() == graphEndpoints[1]) && hasParentAnnotation()) &&
                !(parent != null && getEnd() == parent.getEnd())) {
            getEnd().setTime(endTime);
        }

        // HB, 27-feb-02: correct potential time overlaps
        if ((((TranscriptionImpl) (((TierImpl) getTier()).getParent())).getTimeChangePropagationMode() == Transcription.BULLDOZER) &&
                (!hasParentAnnotation())) {
            ((TierImpl) getTier()).correctOverlapsByPushing(this, oldBegin,
                oldEnd);
        } else if ((((TranscriptionImpl) (((TierImpl) getTier()).getParent())).getTimeChangePropagationMode() == Transcription.SHIFT) &&
                (!hasParentAnnotation())) {
            Vector fixedSlots = new Vector(connectedTimeSlots);
            ((TranscriptionImpl) (((TierImpl) getTier()).getParent())).correctOverlapsByShifting(this,
        ...
        ((TranscriptionImpl) (((TierImpl) getTier()).getParent())).pruneAnnotations(this.getTier());
    ...
    public Annotation getParentAnnotation() {
        ...
        if (parentTier != null) {
            if (((TierImpl) getTier()).getLinguisticType().getConstraints().getStereoType() == 
            ...
            } else if (((TierImpl) getTier()).getLinguisticType().getConstraints().getStereoType() == 
