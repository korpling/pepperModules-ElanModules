...
public class EAF27Encoder implements AnnotationDocEncoder, ServerLogger {
     ...
     */
    public static Element createDOM(Transcription theTranscription, List tierOrder, String path) {
        ...
        HashMap extRefIds = new HashMap();
        ArrayList extRefList = new ArrayList();
        ...
        while (tierIter2.hasNext()) {
            ...
            while (annotIter2.hasNext()) {
                ...
                if (ann instanceof AbstractAnnotation && ((AbstractAnnotation) ann).getExtRefs() != null) {
                	for (int j = 0; j < ((AbstractAnnotation) ann).getExtRefs().size(); j++) {
                		Object thisExtRef = ((AbstractAnnotation) ann).getExtRefs().get(j);
                    ...
                    if (ann instanceof SVGAlignableAnnotation) {
                        if (((SVGAlignableAnnotation) ann).getShape() != null) {
                            ...
                            annSubElement = eafFactory.newAlignableAnnotation(annId,
                                    beginTsId, endTsId, svgId, extRefId);
                        } else {
                            ((SVGAlignableAnnotation) ann).setSVGElementID(null);
                            annSubElement = eafFactory.newAlignableAnnotation(annId,
                                    beginTsId, endTsId, null, extRefId);
                        }
                    } else {
                        annSubElement = eafFactory.newAlignableAnnotation(annId,
                                beginTsId, endTsId, null, extRefId);
        ...
        if (lTypes != null) {
            ...
            while (typeIter.hasNext()) {
                ...
                Element typeElement = eafFactory.newLinguisticType(lt.getLinguisticTypeName(),
                        ...
                        stereotype, lt.getControlledVocabylaryName(), extRefId, 
                        lexRef);
        ...
        if (conVocs.size() > 0) {
            ...
            for (int i = 0; i < conVocs.size(); i++) {
                ...
                if (cv instanceof ExternalCV) {
        			...
        			if (((ExternalCV) cv).getExternalRef() != null) {
        				if ((((ExternalCV) cv).getExternalRef() instanceof String) && ((String)((ExternalCV) cv).getExternalRef()).length() == 0) {
        					extRefId = null;
        				} else if (!extRefIds.containsValue(((ExternalCV) cv).getExternalRef())) {
        					extRefId = "er" + extRefIndex++;
        					extRefIds.put(extRefId, ((ExternalCV) cv).getExternalRef());
        					extRefList.add(extRefId);
        				} else {
        					for (int k = 0; k < extRefList.size(); k++) {
        						if (((ExternalCV) cv).getExternalRef().equals(extRefIds.get(extRefList.get(k)))) {
...
//                        		}
        ...
        for (int i = 0; i < extRefList.size(); i++) {
        	id = (String) extRefList.get(i);
        	eri = (ExternalReferenceImpl) extRefIds.get(id);
        	if (id != null && eri != null) {
        		erElement = eafFactory.newExternalReference(id, types.get(eri.getReferenceType()), eri.getValue());
