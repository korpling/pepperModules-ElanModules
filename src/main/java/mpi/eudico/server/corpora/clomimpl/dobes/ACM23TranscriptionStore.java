...
/**
 * A TranscriptionStore that corresponds to EAF v2.3.<br>
 * Version 2.3 extends v2.2 by adding support for LinkedFileDescriptors and 
 * for a new Constraint stereotype, INCLUDED_IN, which is a subdivision with gaps.
 *
 * @see EAF22TranscriptionStore
 * 
 ...
 */
public class ACM23TranscriptionStore implements TranscriptionStore {
	...
	private static final Logger LOG = Logger.getLogger(
		ACM23TranscriptionStore.class.getName());
	
	private boolean debug = false;
	
    /**
     * Creates a new ACM23TranscriptionStore instance
     */
    public ACM23TranscriptionStore() {
        super();
		//debug = true;
    }
	...
	*/
    ...
    public void storeTranscriptionIn(Transcription theTranscription, EncoderInfo encoderInfo,
		...
		switch (format) {
			case TranscriptionStore.EAF:
				...
				new EAF23Encoder().encodeAndSave(theTranscription, null, tierOrder, path);			
				break;
    ...
    public void storeTranscriptionAsTemplateIn(Transcription theTranscription,
        ...
        new EAF23Encoder().encodeAsTemplateAndSave(theTranscription, tierOrder, path);
    }
    ...
    public void loadTranscription(Transcription theTranscription, DecoderInfo decoderInfo) {
		...
		if (lowerPathName.endsWith("cha")) {
			...
			parser = ParserFactory.getParser(ParserFactory.EAF23);
		}
