...
/**
 * $Id: Transcription2TeX.java 7511 2006-12-06 16:06:07Z klasal $
 * 
...
public class Transcription2TeX {
    ...
    static public void exportTiers(Transcription transcription, 
        ...
        List rootTiers = new ArrayList();

        for (int i = 0; i < tierNames.length; i++) {
            rootTiers.add(transcription.getTierWithId(tierNames[i]));
        }
        ...
        for (int i = 0; i < rootTiers.size(); i++) {
            ...
            List annotations = ((TierImpl) rootTiers.get(i)).getAnnotations();

            int j = 0;

            while (j < annotations.size()) {
                if (annotations.get(j) instanceof Annotation
                        && TimeRelation.overlaps((Annotation) annotations.get(j),
                                beginTime, endTime)) {
                    j++;
                }
                else {
     ...
     * @param rootTiers
     */
    private static void removeDependentTiers(List rootTiers) {
        TierImpl tier1;
        TierImpl tier2;
     ...
     * @throws Exception
     */
    private static boolean hasOnlyLinearDependencies(List rootTiers) {
        TierImpl tier1;
        TierImpl tier2;
    ...
    static private void exportAnnotations(BufferedWriter writer, String[] tierNames,
            List annotations, File exportFile) throws IOException {
        if (exportFile == null) {
            return;
        }
    ...
    static private String getTeXTree(String[] tierNames, Annotation annotation) {
        ...
        List childrenTiers = tier.getChildTiers();
        StringBuffer childrenTree = new StringBuffer();

        try {
            for (int k = 0; k < childrenTiers.size(); k++) {
                String childrenTierName = ((TierImpl) childrenTiers.get(k)).getName();

                for (int j = 0; j < tierNames.length; j++) {
                    if (tierNames[j].equals(childrenTierName)) {
                        List children = annotation
                                .getChildrenOnTier((TierImpl) childrenTiers.get(k));

                        if (children.size() > 0) {
                            for (int i = 0; i < children.size(); i++) {
                                childrenTree.append(getTeXTree(tierNames,
                                        (Annotation) children.get(i)));
                            }
                        }
