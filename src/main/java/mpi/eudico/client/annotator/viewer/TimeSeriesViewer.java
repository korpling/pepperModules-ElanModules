...
public class TimeSeriesViewer extends DefaultTimeScaleBasedViewer
    ...
    private void paintBuffer() {
        ...
        for (int i = 0; i < trackPanels.size(); i++) {
            panel = (TSTrackPanelImpl) trackPanels.get(i);
            y -= panel.getHeight();
            big2d.translate(0, y);
    ...
    public List getTracksForPanel(int panelIndex) {
        ...
        TSTrackPanelImpl panel = (TSTrackPanelImpl) trackPanels.get(panelIndex);
        List tracks = panel.getTracks();
        AbstractTSTrack tr;
        //String[] names = new String[tracks.size()];
        ArrayList names = new ArrayList(tracks.size());
    ...
    protected void updatePopup(Point p) {
        ...
        if (trackManager != null) {
            ArrayList trs = trackManager.getRegisteredTracks();
	    ...
	    Collections.sort(trs);
            ...
            List curTracks = selTrackPanel.getTracks();
    ...
    public void setMediaTimeOffset(long offset) {
        if (offset != mediaTimeOffset) {
            ...
            for (int i = 0; i < trackPanels.size(); i++) {
                panel = (TSTrackPanelImpl) trackPanels.get(i);
                tracks = panel.getTracks();

                for (int j = 0; j < tracks.size(); j++) {
                    track = (AbstractTSTrack) tracks.get(j);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == fitVerticalMI) {
            ...
            paintBuffer();
            updateScrollBar();
            setPreference("TimeSeriesViewer.FitVertically", new Boolean(autoFitVertical),
            		transcription);
            return;
        }
        ...
        if (e.getSource() == showTrackValueMenu) {
        	...
        	showTrackValueMenu.setSelected(showTrackValues);
        	paintBuffer();
        	setPreference("TimeSeriesViewer.ShowTrackValues", new Boolean(showTrackValues),
    				transcription);
        	return;
        }
        ...
        if (e.getSource() == timeRulerVisMI) {
        	...
        	adjustPanelHeight();
        	paintBuffer();
        	setPreference("TimeSeriesViewer.TimeRulerVisible", new Boolean(timeRulerVisible),
    				transcription);
        	return;
        }
        ...
        if (e.getSource() == addAllTrackMI) {
            if (selTrackPanel != null) {
                ArrayList tracks = trackManager.getRegisteredTracks();
                AbstractTSTrack tr;
        ...
        if (e.getSource() == removeAllTrackMI) {
            if (selTrackPanel != null) {
                List tracks = selTrackPanel.getTracks();

                for (int i = tracks.size() - 1; i >= 0; i--) {
                    selTrackPanel.removeTrack((AbstractTSTrack) tracks.get(i));
                }
		...
		if (e.getSource() == addPanelForEachTrackMI) {
		    ...
		    for (int i = 0; i < totPanels; i++) {
		        ...
		        TSTrackPanelImpl tsp = (TSTrackPanelImpl) trackPanels.get(i);
	...
	public void preferencesChanged() {
		Integer numPanels = (Integer) getPreference("TimeSeriesViewer.NumberOfPanels",
				transcription);
		if (numPanels != null) {
			...
			for (int i = 1; i <= np; i++) {// 1 based
				trackNames = (List) getPreference("TimeSeriesViewer.Panel-" + i,
						transcription);
				setTracksForPanel(i - 1, trackNames);
			}
		}
        Rectangle dialogBounds = (Rectangle) Preferences.get("TimeSeriesViewer.Detached.Bounds",
                transcription);
		...
		Object zoomLevel = getPreference("TimeSeriesViewer.ZoomLevel",
				transcription);
		...
		Boolean fitVert = (Boolean) getPreference("TimeSeriesViewer.FitVertically",
				transcription);
		if (fitVert != null) {
		...
		Boolean rulerVis = (Boolean) getPreference("TimeSeriesViewer.TimeRulerVisible",
				transcription);
		if (rulerVis != null) {
		...
		Boolean showTV = (Boolean) getPreference("TimeSeriesViewer.ShowTrackValues", transcription);
	...
	private void storePanelSetup() {
		...
		if (np > 0) {
			setPreference("TimeSeriesViewer.NumberOfPanels", new Integer(np),
					transcription);
			List names = null;
			for (int i = 0; i < np; i++) {
    ...
    private void testTrack() {
        ...
        try {
            ...
            if (trackPanels.size() > 0) {
                TSTrackPanelImpl tsp = (TSTrackPanelImpl) trackPanels.get(0);
    ...
    public void timeSeriesChanged(TimeSeriesChangeEvent event) {
    	if (event.getEditSourceType() == TimeSeriesChangeEvent.TRACK_AND_PANEL) {
    		if (event.getEditType() == TimeSeriesChangeEvent.ADD) {
            	...
            	if (trackPanels.size() == 1 && ((TSTrackPanelImpl) trackPanels.get(0)).getTracks().size() == 0) {
            		// the first track
            		TSTrackPanelImpl trsp = (TSTrackPanelImpl) trackPanels.get(0);
            		trsp.addTrack(tr);
    	...
    	if (event.getEditSourceType() == TimeSeriesChangeEvent.TRACK) {
            if (event.getSource() instanceof TSTrackConfiguration ||
                ...
                for (int i = 0; i < trackPanels.size(); i++) {
                    pan = (TSTrackPanelImpl) trackPanels.get(i);

                    List tracks = pan.getTracks();

                    for (int j = 0; j < tracks.size(); j++) {
                        tr2 = (AbstractTSTrack) tracks.get(j);
                        ...
                        if (tr2 == tr) {
                            if (event.getEditType() == TimeSeriesChangeEvent.CHANGE) {
                                ...
                                if (active && (pan.getTracks().size() > 0)) {
                                    pan.getRuler()
                                       .setRange(((AbstractTSTrack) tracks.get(
                                            0)).getRange());
                                    pan.getRuler()
                                       .setUnitString(((AbstractTSTrack) tracks.get(
                                            0)).getUnitString());
                                }
