...
MultiTierViewer, AdjustmentListener, ACMEditListener, ChangeListener {
	...
	private void initViewer2() {
        ...
        for (int i = 0; i < ks.length; i++) {
        	if (ks[i].getKeyCode() == KeyEvent.VK_LEFT || ks[i].getKeyCode() == KeyEvent.VK_RIGHT) {
        		continue;
        	}
        	int condition = zoomSlider.getConditionForKeyStroke(ks[i]);
        	im = zoomSlider.getInputMap(condition);
        	if (im != null) {       		
        		Object obj = im.get(ks[i]);
        		am.remove(obj);
        ...
        actionMap.put(ELANCommandFactory.SEGMENT, new SegmentAction());  
	...
	private void updateShortcutMaps(){
		...
		while(it.hasNext()){
			Map.Entry<String, Action> pair = (Entry<String, Action>) it.next();
...
//        	editTier2d = allTiers.get(0);
    ...
    private void reextractTier(Tier2D tier2d) {
        ...
        Iterator annotIter = tier2d.getTier().getAnnotations().iterator();
        Annotation a;
    ...
    private void paintBuffer() {
        ...
        synchronized (paintlock) {
	        ...
	        if (editTier2d != null) {
	            Iterator tagIt = editTier2d.getTags();
...
//        }       
	...
	protected void updatePopup(Point p) {
		...
		deleteAnnoMI.setEnabled(dragEditTag2D != null);
 	  	   		...
 	  	   		if(childTiers!= null){
 	  	   			...
 	  	   			if(!valid){
 	  	   				...
 	  	   				for(int i=0;i < dependentTiers.size();i++){
 	  	   					...
 	  	   					dependingAnnotations.addAll(aa.getChildrenOnTier((Tier) dependentTiers.get(i)));
 	  	   				}	
    ...
    public void setSegmentationMode(int nextMode) {
    	...
    	// hier stop player??
    	if (nextMode >= TWO_TIMES_SEGMENTATION && nextMode <= ONE_TIME_FIXED_SEGMENTATION) {
    		segmentMode = nextMode;
    		// hier reset data fields??
            timeCount = 0;
            lastSegmentTime = -1;
    	}
    }
...
//            System.out.println("IE time: " + intervalEndTime);
        ...
        case ACMEditEvent.ADD_ANNOTATION_HERE:
            ...
            if (e.getInvalidatedObject() instanceof TierImpl &&
            	...
            	if (lastRecord != null) {
            		...
            		Object[] args = new Object[] { "", lastRecord.getValue(), lastRecord.getExtRef() };
            		c.execute((Annotation) e.getModification(), args);
            		lastRecord = null;
            	}
            }
	 ...
	 */
	private void annotationAdded(TierImpl tier, Annotation ann) {
		Tier2D tier2d;
		for (int i = 0; i < allTiers.size(); i++) {
			...
			if (tier2d.getTier() == tier) {
				...
				List<Tag2D> annos = (List<Tag2D>) tier2d.getTagsList();
				int size = annos.size();
				Tag2D tag2d;
	...
	public void preferencesChanged() {
		Integer fontSize = (Integer) getPreference("SegmentationViewer.FontSize", 
				transcription);
		if (fontSize != null) {
			setFontSize(fontSize.intValue());
		}
		
		Object zoomLevel = getPreference("SegmentationViewer.ZoomLevel", 
				transcription);
		...
		Object rulerVisible = getPreference("SegmentationViewer.TimeRulerVisible", transcription);
		if (rulerVisible instanceof Boolean) {
			setTimeRulerVisible((Boolean) rulerVisible);
	...
	protected void setTimeRulerVisible(boolean visible) {
		...
		paintBuffer();
		notifySegmentationControlPanel();
		setPreference("SegmentationViewer.TimeRulerVisible", new Boolean(timeRulerVisible), 
				transcription);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().indexOf("font") > -1) {
            ...
            setPreference("SegmentationViewer.FontSize", new Integer(size), 
            		transcription);
            return;
        } 
...
//        } 
	...
	private Tag2D getAnnotationAtX(int x) {
		...
		for (int i = 0; i < editTier2d.getTagsList().size(); i++) {
			t2d = (Tag2D) editTier2d.getTagsList().get(i);
...
//        }
     ...
     */
    private void deleteEditAnnotation() {
    	if (dragEditTag2D == null) {
    		return;
    	}
    	...
    	TierImpl tier = (TierImpl) editAnn.getTier();
    	dragEditTag2D = null;
	...
	public void mergeAnnotation(boolean next) {
		if (dragEditTag2D != null) {
            Annotation aa = dragEditTag2D.getAnnotation();
            TierImpl tier = (TierImpl) aa.getTier();  
        	Command c;
	...
	public class DeleteAnnotationAction extends AbstractAction {
		...
		public DeleteAnnotationAction() {
			...
			putValue(Action.ACCELERATOR_KEY, 
					ShortcutsUtil.getInstance().getKeyStrokeForAction(ELANCommandFactory.DELETE_ANNOTATION, ELANCommandFactory.SEGMENTATION_MODE));   
            putValue(Action.DEFAULT, null);
		}
	...
	public class SplitAnnotationAction extends AbstractAction {		
		...
		public SplitAnnotationAction() {
			...
			putValue(Action.ACCELERATOR_KEY, 
		            		ShortcutsUtil.getInstance().getKeyStrokeForAction(ELANCommandFactory.SPLIT_ANNOTATION, ELANCommandFactory.SEGMENTATION_MODE));   
			putValue(Action.DEFAULT, null);	
		}
