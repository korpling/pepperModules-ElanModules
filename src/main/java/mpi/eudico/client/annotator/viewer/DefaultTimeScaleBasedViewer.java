    ...
    protected void recalculateInterval(final long mediaTime) {
        ...
        if (playerIsPlaying()) {
            ...
            if (mediaTime > intervalEndTime) {
                ...
                newEndTime = newBeginTime + (int) (intervalWidth * msPerPixel);

                while ((newEndTime += (intervalWidth + msPerPixel)) < mediaTime) {
                    newBeginTime += (intervalWidth * msPerPixel);
                }
            } else if (mediaTime < intervalBeginTime) {
                newEndTime = intervalBeginTime;
                newBeginTime = newEndTime - (int) (intervalWidth * msPerPixel);
                ...
                if (newBeginTime < 0) {
                    newBeginTime = 0;
                    newEndTime = (long) (intervalWidth * msPerPixel);
                }
            } else {
