...
public class ClipMediaCommand implements Command {
		...
		if (arguments[0] instanceof ViewerManager2) { 				
			...
			if (arguments.length > 1) {
				...
				if (selection != null && timeBegin == -1) {
					...
					Object promptObj = Preferences.get("Media.PromptForFilename", viewerManager.getTranscription());
					if (promptObj instanceof Boolean) {
						promptForFile = (Boolean) promptObj;
					}
				...
				Object mmOnlyObj = Preferences.get("Media.OnlyClipFirstMediaFile", viewerManager.getTranscription());
				if (mmOnlyObj instanceof Boolean) {
					masterMediaOnly = (Boolean) mmOnlyObj;
				}
				
				Object runAsync = Preferences.get("Media.ClipInParallel", viewerManager.getTranscription());
				if (runAsync instanceof Boolean) {
					asynchronous = (Boolean) runAsync; 
				}
				// check for audio or video?
				List players = viewerManager.getTranscription().getMediaDescriptors();
	...
	/**
	 * Constructsa destination name based on source name and a general destination name.
	 * 
		...
		public void run() {
			...
			while (reader != null && !isInterrupted()) {
				try {
					...
					if (reader.ready()) {
						ClientLogger.LOG.info(reader.readLine());
					} else {
