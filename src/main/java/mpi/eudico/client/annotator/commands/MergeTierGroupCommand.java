...
public class MergeTierGroupCommand implements UndoableCommand {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        TierImpl sourceTier1 = (TierImpl) transcription.getTierWithId(sourceTierName1);
        TierImpl sourceTier2 = (TierImpl) transcription.getTierWithId(sourceTierName2);
        ...
        TierImpl destTier = (TierImpl) transcription.getTierWithId(sourceTierName1 + copySuffix);
        ...
        if (destTier != null) {
            ...
            while (destTier != null) {
                cName = cName + count;
                destTier = (TierImpl) transcription.getTierWithId(sourceTierName1 + cName);
                count++;
            }
        ...
        if ((transcription != null) && (destTierName != null)) {
            ...
            for (int i = 0; i < tierRecords.size(); i++) {
            	...
            	if (transcription.getTierWithId(tr.getName()) == null) {
            		if (tr.getParentTier() != null) {
            			pt = (TierImpl) transcription.getTierWithId(tr.getParentTier());
            			if (pt == null) {
            				// error situation
            ...
            if ((annRecords != null) && (annRecords.size() > 0)) {
                ...
                for (int i = 0; i < annRecords.size(); i++) {
                    node = (DefaultMutableTreeNode) annRecords.get(i);
                    ann = AnnotationRecreator.createAnnotationFromTree(transcription, node);
                    if (ann == null) {
