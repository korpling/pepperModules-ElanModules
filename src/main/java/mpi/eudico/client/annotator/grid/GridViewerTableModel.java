...
public class GridViewerTableModel extends AbstractTableModel {
 ...
 //   private final Hashtable numberHash = new Hashtable();
    ...
    /*
     * a table containing the Vectors of annotations/empty slots of each child
    ...
    public AnnotationCore getAnnotationCore(int row) {
        return ((AnnotationCore) annotations.get(row));
    }
    ...
    public boolean isCellEditable(int row, int column) {
        ...
        !Arrays.asList(fixedColumnNames).contains(getColumnName(column));
    }
    ...
    public String getColumnName(int nr) {
        if ((0 <= nr) && (nr < usedColumnNames.size())) {
            return (String) usedColumnNames.get(nr);
        }
            ...
            if (size > 0) {
                if ((column > 2) && (column <= (2 + size)) && (row < getRowCount())) {
                    ...
                    if (childTierHash.containsKey(tierName)) {
                        List anns = (List) childTierHash.get(tierName);
        ...
        try {
            ...
            if (cn.equals(BEGINTIME)) {
                return getStringRepresentation(
                    ((AnnotationCore) annotations.get(row)).getBeginTimeBoundary()); //begintime
            }

            if (cn.equals(ENDTIME)) {
                return getStringRepresentation(
                    ((AnnotationCore) annotations.get(row)).getEndTimeBoundary()); //endtime
            }

            if (cn.equals(DURATION)) {
                AnnotationCore ann = (AnnotationCore) annotations.get(row);
            ...
            if (childTierHash.containsKey(cn)) {
                List v = (List) childTierHash.get(cn);
    ...
    /**
     * Replaces the Vector of annotations of the current tier. Called when the parent tier has not
    ...
    private void removeChildTiers() {
        ...
        if (numChildTiers > 0) {
            Enumeration en = childTierHash.keys();

            while (en.hasMoreElements()) {
                usedColumnNames.remove(en.nextElement());
            }
