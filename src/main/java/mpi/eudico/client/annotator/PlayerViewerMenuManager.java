...
public class PlayerViewerMenuManager {
    ...
    public List getStoredVisiblePlayers() {
        ...
        ArrayList tempList = (ArrayList) Preferences.get(HIDDEN_PLAYERS_KEY,
                transcription);

        if (tempList == null) {
            return transcription.getMediaDescriptors();
        } else {
            int size = transcription.getMediaDescriptors().size();
            ArrayList visList = new ArrayList(size);
            MediaDescriptor md;

            for (int i = 0; i < size; i++) {
                md = (MediaDescriptor) transcription.getMediaDescriptors().get(i);
    ...
    public void initPlayerMenu() {
        List visuals = frame.getLayoutManager().getVisualPlayers();
        List descriptors = transcription.getMediaDescriptors();
        MediaDescriptor md;
        String fileName;
        int visibles = 0;
        ...
        for (int i = 0; i < descriptors.size(); i++) {
            ...
            if (curVisual && (visibles < 4)) {
                ...
                menuPlayers.add(new SelectableObject(md, true));
                visibles++;
            } else {
                menuPlayers.add(new SelectableObject(md, false));
            }
        }
    }
    ...
    private void removePlayer(MediaDescriptor md) {
        ...
        if ((otherMd != null) && md.mediaURL.equals(otherMd.mediaURL)) {
            ...
            List slavePlayers = frame.getViewerManager().getSlaveMediaPlayers();
            ...
            if (slavePlayers.size() == 0) {
                ...
                frame.getViewerManager()
                     .setMasterMediaPlayer((ElanMediaPlayer) slavePlayers.get(0));
                frame.getViewerManager().destroyMediaPlayer(player);
                frame.getLayoutManager().remove(player);
                frame.getViewerManager().getMasterMediaPlayer()
                     .setMediaTime(mediaTime);
...
slaveloop:  // find the first visual non empty media player
                    for (int i = 1; i < slavePlayers.size(); i++) {
                        if (!(slavePlayers.get(i) instanceof EmptyMediaPlayer)) {
                            nextPl = (ElanMediaPlayer) slavePlayers.get(i);      
