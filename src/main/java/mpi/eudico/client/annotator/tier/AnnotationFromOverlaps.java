...
public class AnnotationFromOverlaps implements ClientLogger, ProcessReporter {
     ...
     *        	<li>arg[9] = the overlaps criteria (0 = same value, 1 = different value, 3 = based on constraints) (List<String[]>)
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        transcriptionMode = (Integer) arguments[10];
        ...
        report = new SimpleReport(ElanLocale.getString(
               "OverlapsDialog.Report.Title"));
        
        int noOfFiles = 1;
        if(transcriptionMode == MULTI_FILE_MODE){
    ...
    private boolean createAnnotations(TranscriptionImpl transcription) {    	
    	...
    	if(parentTier != null){
    		...
    		for(int i = 0 ; i < tierList.size(); i++ ){
    			if(i == 0){
    				participant = tierList.get(i).getParticipant();
    				locale = tierList.get(i).getDefaultLocale();
    			}else {
    			...
    			if(!checkLocale && !checkParticpant){
    				break;
    			}
    		}
    	}
        ...
        ArrayList<AnnotationValuesRecord> annValRecordList = getComputedAnnRecord(tierList, transcription );
        ...
        float perAnn = 25f;
        if(transcriptionMode == this.SINGLE_FILE_MODE){
        ...
        for( int i=0; i<annValRecordList.size(); i++ ){
        	...
        	if (aa != null) {
        		...
        		if(transcriptionMode == SINGLE_FILE_MODE){
    ...
    private String getTimeString(long time) {
        ...
        } else if (Constants.HHMMSSFF_STRING.equals(timeFormat)) {
        	if( usePalFormat )
    ...
    protected ArrayList<AnnotationValuesRecord> getComputedAnnRecord( ArrayList<TierImpl> tierList, TranscriptionImpl transcription){
    	...
    	for( int i=0; i<annotations.size(); i++ ){
    		...
    		if( overlapsCriteria != OVERLAP_CRITERIA_USE_SPECIFIED_CONSTRAINTS || (constraint != null && constraint.equals(abstrAnnotation.getValue())) ){   
     ...
     * ovelaps and  creates a new annotation on a new tier, with the
    ...
    private class CalcOverLapsThread extends Thread {
        ...
        public void run() {
        	...
        	else if( transcriptionMode == MULTI_FILE_MODE ){
        		...
        		for( int i = 0; i< filenames.length; i++){
        			...
        			try{
        				if(annotationsCreated){
        					ACMTranscriptionStore.getCurrentTranscriptionStore().storeTranscriptionIn(t, null, new Vector(), directoryToSave, TranscriptionStore.EAF);
        					numChanged++;
        				}
