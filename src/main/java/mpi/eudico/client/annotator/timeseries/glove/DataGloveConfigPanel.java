...
public class DataGloveConfigPanel extends TSConfigPanel
    ...
    private void initComponents() {
        ...
        rowSpinModel.setMinimum(new Integer(0));
        rowSpinModel.setMaximum(new Integer(DataGloveConstants.MAX_NUM_ROWS -
                1));
        ...
        colSpinModel.setMinimum(new Integer(0));
        colSpinModel.setMaximum(new Integer(DataGloveConstants.COLS_PER_ROW[0] - 1));
    ...
    private void updateForTrack(String trackName) {
        if (trackName == null) {
            ...
            colSpinModel.setMaximum(new Integer(
                    DataGloveConstants.COLS_PER_ROW[0] - 1));
            colSpinner.setValue(new Integer(0));
        ...
        for (int i = 0; i < tracks.size(); i++) {
            tst = (TSTrackConfiguration) tracks.get(i);
        ...
        if (currentTrack != null) {
            ...
            if (spos != null) {
                ...
                if (rows.length == 1) {
                    singleCellRB.setSelected(true);
                    rowSpinner.setValue(new Integer(rows[0]));
                    colSpinModel.setMaximum(new Integer(
                            DataGloveConstants.COLS_PER_ROW[rows[0]] - 1));
                    colSpinner.setValue(new Integer(cols[0]));
                } else {
    ...
    private void reextractTracks() {
        ...
        for (int i = 0; i < tracks.size(); i++) {
            tr = (TSTrackConfiguration) tracks.get(i);
    ...
    private void notifyListeners(TimeSeriesChangeEvent ev) {
        if (listeners != null) {
            for (int i = 0; i < listeners.size(); i++) {
                ((TimeSeriesChangeListener) listeners.get(i)).timeSeriesChanged(ev);
            }
        }
    }
