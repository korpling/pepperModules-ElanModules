...
public class SilenceRecognizer implements Recognizer { 
	...
	public void recog() {
		...
		getReportBuilder().append(noiseThreshold1);
		getReportBuilder().append('\n');
		if (lookAtChannel2) {
			getReportBuilder().append("Noise level 2:\t");
			getReportBuilder().append(noiseThreshold2);
			getReportBuilder().append("\n\n");
		}
...
//		}
		...
		if (lookAtChannel1) {
			...
			getReportBuilder().append(segments.size());
			getReportBuilder().append('\n');
		}
		if (lookAtChannel2) {
		...
		getReportBuilder().append("\nProcessing took:\t");
		getReportBuilder().append(System.currentTimeMillis() - curTime);
		getReportBuilder().append(" ms\n");
