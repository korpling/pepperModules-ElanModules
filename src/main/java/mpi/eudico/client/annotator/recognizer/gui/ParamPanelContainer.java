...
public class ParamPanelContainer extends JPanel implements ParamPreferences {
	...
	public ParamPanelContainer(String recogizerName, List<Param> params, List<String> supportedMediaFiles, ViewerManager2 vm, int mode) {
		...
		if (params != null) {
			...
			List list = vm.getTranscription().getTiers();
			if(list !=null){
				for(Object tier : list){
					tiers.add(((Tier)tier).getName());
				}
			}
	...
	private void loadParameterPanels(List<Param> params, int mode){		
		...
		for (Param p : params) {
			if (p instanceof NumParam) {	
				...
				tierPanel = null;
				if(((FileParam) p).contentType == FileParam.AUDIO || (((FileParam) p).contentType == FileParam.VIDEO) ) {				
					panel = new FileParamPanel((FileParam) p, supportedMediaFiles);		
					mediaUpdatePanels.add((FileParamPanel) panel);
					if(!((FileParam) p).optional && supportedMediaFiles.size() <= 0){
						startReg = false;
					}
				} else if( ((FileParam) p).ioType == FileParam.IN && 
						(((FileParam)p).contentType == FileParam.CSV_TIER || ((FileParam)p).contentType == FileParam.TIER || ((FileParam)p).contentType == FileParam.MULTITIER)) {	
	...
	private void showAdvanceParametersDialog(){
		if(advancedDialog == null){
			...
			gbc.weightx = 1.0;
			gbc.weighty = 1.0;
			advancedDialog.getContentPane().add(new JScrollPane(getAdvacnedParameterPanel()), gbc);
