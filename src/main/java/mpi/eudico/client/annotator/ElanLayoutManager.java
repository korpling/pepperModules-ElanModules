...
//    	}         	
    	...
    	if (plModel.isVisual() && plModel.isAttached()) {
    		// this also initializes CocoaQT players
...
//    	}
    	...
    	if (player.getMediaDescriptor() != null) {
	    	Object ar = Preferences.get("AspectRatio(" + player.getMediaDescriptor().mediaURL + ")", 
	    			viewerManager.getTranscription());
	    	if (ar instanceof Float) {
			...
			for (int i = 0; i < playerList.size(); i++) {
				...
				if (model.visualComponent == object) {
					if (model.isVisual() && !model.isAttached()) {
						...
						//InputMap iMap = model.detachedFrame.getJMenuBar().getInputMap(JComponent. WHEN_FOCUSED);
						...
						if (model.player instanceof QTMediaPlayer) {
			    			...
			    			//getElanFrame().getJMenuBar().setInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW, iMap);
			    			//getElanFrame().getJMenuBar().setActionMap(aMap);
						...
						// Cocoa Media player
//			    		if (model.player instanceof CocoaQTMediaPlayer) {
//			    			getElanFrame().getJMenuBar().setInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW, iMap);
//			    			getElanFrame().getJMenuBar().setActionMap(aMap);
//			    		}
    ...
    public void changeMode(int mode){       
    	...
    	if(activeManager != null){
    		...
    		setPreference("LayoutManager.CurrentMode", 
    				new Integer(this.mode), viewerManager.getTranscription()); 
    		doLayout();
    	}
    }
	...
	public List getVisualPlayers() {
		ArrayList visuals = new ArrayList(playerList.size());
	...
	public void setMediaAreaWidth(int mediaAreaWidth) {
        ...
        setPreference("LayoutManager.MediaAreaWidth", 
        		new Integer(this.mediaAreaWidth), viewerManager.getTranscription());
        doLayout();
	}
    ...
    public void setMediaAreaHeight(int mediaAreaHeight) {
        ...
        setPreference("LayoutManager.MediaAreaHeight", 
        		new Integer(this.mediaAreaHeight), viewerManager.getTranscription());      
        doLayout();
    }
	...
	public void setMultiTierControlPanelWidth(int multiTierControlPanelWidth) {
        ...
        setPreference("LayoutManager.ControlPanelWidth", 
        		new Integer(this.multiTierControlPanelWidth), viewerManager.getTranscription());
	...
	private void loadMediaSizePreferences(){
		...
		Integer medWidth = (Integer) Preferences.get("LayoutManager.MediaAreaWidth", viewerManager.getTranscription());
	...
	public void preferencesChanged() {
		...
		Integer cpWidth = (Integer) Preferences.get("LayoutManager.ControlPanelWidth", 
				viewerManager.getTranscription());
		if (cpWidth != null && cpWidth.intValue() >= MIN_MEDIA_HEIGHT) {
			multiTierControlPanelWidth = cpWidth.intValue();			
		}		
		
		Boolean ttShown = (Boolean) Preferences.get("UI.ToolTips.Enabled", null);
