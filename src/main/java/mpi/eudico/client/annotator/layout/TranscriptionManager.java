...
public class TranscriptionManager implements ModeLayoutManager, ActionListener {
	...
	public void initComponents() {
		...
		Object zoom = Preferences.get("SignalViewer.ZoomLevel",transcription);
		if (zoom instanceof Float) {
			zoomLevelSignalViewer = (Float) zoom;
		}else {
	    ...
	    Object val = Preferences.get("TranscriptionManager.PlaceVideoInRight", transcription);
	    if(val != null){
	    	if((Boolean)val){
	    		alignVideo();
	    	}
    ...
    private void loadPreferences(){		
		...
		if(val != null){
			moveViaColumnCB.setSelected((Boolean) val);	
			transViewer.moveViaColumn((Boolean) val);
		}	
		
		val = Preferences.get("TranscriptionManager.AutoPlayBack", null);
		if(val != null){
			autoPlayBackCB.setSelected((Boolean) val);
			transViewer.setAutoPlayBack((Boolean) val);
		}	
		
		val = Preferences.get("TranscriptionManager.AutoCreateAnnotation", null);
		if(val != null){
			autoCreateAnnCB.setSelected((Boolean) val);
			transViewer.autoCreateAnnotations((Boolean) val);
		}
		
		val = Preferences.get("TranscriptionManager.ScrollToCenter", null);
		if(val != null){
			showActiveCellInCenterCB.setSelected((Boolean) val);
			transViewer.scrollActiveCellInCenter((Boolean) val);
		}
		
		val = Preferences.get("TranscriptionManager.ShowTierNames", null);
		if(val != null){
			showTierNamesCB.setSelected((Boolean) val);
			transViewer.showTierNames((Boolean)val);
		...
		val = Preferences.get("TranscriptionManager.ShowColorOnlyOnNoColumn", null);
		if(val != null){
			showColorOnlyOnNoColumnCB.setSelected((Boolean) val);
			transViewer.showColorOnlyOnNoColumn((Boolean)val);
		}				
		 
		val = Preferences.get("TranscriptionTable.ColumnTypes", transcription);
		List<String> types = null;
		if(val instanceof List){
			types = (List<String>)val;
		} else {
			List<String> colTypes = new ArrayList<String>();
			val = Preferences.get("TranscriptionTable.Column1Type", transcription);
			if(val != null){
				colTypes.add(val.toString());
				val = Preferences.get("TranscriptionTable.Column2Type", transcription);
				if(val != null){
					colTypes.add(val.toString());
					val = Preferences.get("TranscriptionTable.Column3Type", transcription);
					if(val != null){
						colTypes.add(val.toString());	
					}
				}
		...
		if(types != null && types.size() > 0){
			PossibleTypesExtractor computeTypes = new PossibleTypesExtractor(viewerManager.getTranscription());
		...
		if(validTypes == null || validTypes.size() <= 0){
			TranscriptionModeSettingsDlg settingsDialog = new TranscriptionModeSettingsDlg(layoutManager, null,null, null, (Integer)val);
			...
			HashMap<TierImpl, List<TierImpl>> tierMap = null;
			List<String> hiddenTiersList = null;
			HashMap<String, List<String>> map = (HashMap<String, List<String>>) Preferences.get("TranscriptionTable.TierMap", viewerManager.getTranscription());
			if(map != null){			
				tierMap = getValidatedTierMap(map, deletedTypeIndexList);	
			}
			
			if(tierMap != null){
				List<String> valList = (List<String>) Preferences.get("TranscriptionTable.HiddenTiers", viewerManager.getTranscription());
				if(valList != null){
					hiddenTiersList = getValidatedTierList(valList);			
				}
				
				valList = (List<String>) Preferences.get("TranscriptionTable.NonEditableTiers", viewerManager.getTranscription());
				if(valList != null){			
			...
			applyChanges(tierMap, hiddenTiersList, validTypes, (Integer)val, true);
			transViewer.updateTable();
		}
    }
    
    private HashMap<TierImpl, List<TierImpl>> getValidatedTierMap(HashMap<String, List<String>> map, List<Integer> deletedTypeIndexList){
	...
	public void actionPerformed(ActionEvent e) {			
		if (e.getSource() == butAlignVideoToRight){
			...
			if(viewerManager.getMasterMediaPlayer().isPlaying()){
				viewerManager.getMasterMediaPlayer().start();
			}					
	...
	public void isClosing() {	
		...
		if(signalViewer != null){	
			...
			Object zoom = Preferences.get("SignalViewer.ZoomLevel",transcription);
			if (zoom instanceof Float) {
				setPreference("TM.SignalViewer.ZoomLevel", new Float((Float)zoom), transcription);
			}
			
			if(zoomLevelSignalViewer > -1F){
				setPreference("SignalViewer.ZoomLevel", zoomLevelSignalViewer, transcription);
			} else {
				setPreference("SignalViewer.ZoomLevel", new Float(100.0f), transcription);
			}
