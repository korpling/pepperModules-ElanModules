...
ListSelectionListener, ChangeListener, ItemListener, TableModelListener {
	...
	protected void initComponents() {	
        ...
        typeModel = new TierExportTableModel();
        JTable typeTable = new JTable(typeModel);
        
        typeModel.setColumnIdentifiers(new String[] { SELECT_COLUMN, TIER_NAME_COLUMN });
        typeTable.getColumn(SELECT_COLUMN)
                 .setCellEditor(new DefaultCellEditor(new JCheckBox()));
        typeTable.getColumn(SELECT_COLUMN)
                 .setCellRenderer(new CheckBoxTableCellRenderer());
        typeTable.getColumn(SELECT_COLUMN).setMaxWidth(30);
        typeTable.setShowVerticalLines(false);
        typeTable.setTableHeader(null);
        
        partModel = new TierExportTableModel();
        JTable partTable = new JTable(partModel);
        
        partModel.setColumnIdentifiers(new String[] { SELECT_COLUMN, TIER_NAME_COLUMN });
        partTable.getColumn(SELECT_COLUMN)
                 .setCellEditor(new DefaultCellEditor(new JCheckBox()));
        partTable.getColumn(SELECT_COLUMN)
                 .setCellRenderer(new CheckBoxTableCellRenderer());
        partTable.getColumn(SELECT_COLUMN).setMaxWidth(30);
        partTable.setShowVerticalLines(false);
        partTable.setTableHeader(null);
        
        annotModel = new TierExportTableModel();
        JTable annotTable = new JTable(annotModel);
        
        annotModel.setColumnIdentifiers(new String[] { SELECT_COLUMN, TIER_NAME_COLUMN });
        annotTable.getColumn(SELECT_COLUMN)
                 .setCellEditor(new DefaultCellEditor(new JCheckBox()));
        annotTable.getColumn(SELECT_COLUMN)
                 .setCellRenderer(new CheckBoxTableCellRenderer());
        annotTable.getColumn(SELECT_COLUMN).setMaxWidth(30);
        annotTable.setShowVerticalLines(false);
        annotTable.setTableHeader(null);
...
//		}
		...
		for (int i = 0; i < newTierOrder.size(); i++) {
			...
			if (allTierNames.contains(name)) {			
				if (curOrder.contains(name)) {
					model.addRow(new Object[]{currentSelected.get(name), name});
				} else {
		...
		for(String s : curOrder) {
			if (!newTierOrder.contains(s)) {
				model.addRow(new Object[]{currentSelected.get(s), s});				
			}
		}
    ...
    public List<String> getSelectedItems() {
		...
		DefaultTableModel iterModel;
		if (BY_ANN == selectionMode) {
			iterModel = annotModel;
		} else if (BY_PART == selectionMode) {
			iterModel = partModel;
		} else if (BY_TYPE == selectionMode) {
			iterModel = typeModel;
		} else {
			iterModel = model;
		}
