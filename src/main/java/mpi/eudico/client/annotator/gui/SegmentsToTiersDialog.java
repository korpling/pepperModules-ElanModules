...
public class SegmentsToTiersDialog extends ClosableDialog
     ...
     */
    public SegmentsToTiersDialog(TranscriptionImpl transcription,
        List segmentations) throws HeadlessException {
        super();
     ...
     */
    public SegmentsToTiersDialog(Frame owner, TranscriptionImpl transcription,
        List segmentations) throws HeadlessException {
        super(owner, true);
        this.transcription = transcription;
        this.segmentations = segmentations;
    ...
    private void extractSegmentations() {
        if ((segmentations != null) && (segmentations.size() > 0)) {
            ...
            for (int i = 0; i < segmentations.size(); i++) {
                seg = (Segmentation) segmentations.get(i);
                names[i] = seg.getName();
        ...
        if (allSegmentationsCombo.getSelectedIndex() > -1) {
            ...
            for (int i = 0; i < segmentations.size(); i++) {
                ...
                if (name.equals(seg.getName())) {
                    List segments = seg.getSegments();
                    Segment segment = null;
                    List segLabels = new ArrayList();
    ...
    private void convertSingleSegmentation() {
        ...
        for (int i = 0; i < segmentations.size(); i++) {
            if (((Segmentation) segmentations.get(i)).getName()
        ...
        HashMap labelMap = new HashMap(segmentsTable.getRowCount());
        ...
        HashMap numMap = new HashMap(segmentsTable.getRowCount());
        Boolean sel;
        ...
        ArrayList records = new ArrayList(segments.size());
        Segment seg = null;
        String val = null;
        int[] curCount = null;
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            if (seg.label == null) {
                val = (String) labelMap.get(SegmentTableModel.NULL);
            } else {
                val = (String) labelMap.get(seg.label);
            }
            ...
            if (val != null) {
                ...
                if (seg.label == null) {
                    curCount = (int[]) numMap.get(SegmentTableModel.NULL);
                } else {
                    curCount = (int[]) numMap.get(seg.label);
                }
        ...
        HashMap segmentationMap = new HashMap(1);
    ...
    private void convertMultiSegmentation() {
        ArrayList selSeg = new ArrayList();
        Segmentation seg = null;
        String name = null;
        Boolean sel;
        ...
        for (int i = 0; i < segmentationsTable.getRowCount(); i++) {
            ...
            if (sel.booleanValue()) {
                selSeg.add(dtModel.getValueAt(i,
        ...
        for (int i = 0; i < selSeg.size(); i++) {
            ...
            for (int j = 0; j < segmentations.size(); j++) {
                if (((Segmentation) segmentations.get(j)).getName().equals(name)) {
                    seg = (Segmentation) segmentations.get(j);
    ...
    /*
       public void stateChanged(ChangeEvent e) {
           if (e.getSource() == editCB) {
               model.setValueAt(new Boolean(editCB.isSelected()),
