...
public class ActivityMonitoringDialog extends ClosableDialog implements ActionListener, ChangeListener{
	...
	private JPanel getAdvancedOptionsPanel(){
    	...
    	Object val = Preferences.get("ActivityMonitoring.FilesPerSession", null);
    	if(val instanceof Boolean ){
    		filesPerSessionCB.setSelected(((Boolean) val).booleanValue());
    	}
    	...
    	val = Preferences.get("ActivityMonitoring.AppendToFile", null);
    	if(val instanceof Boolean ){
    		appendToFileCB.setSelected(((Boolean) val).booleanValue());
    	} 
    	
    	if(filesPerSessionCB.isSelected()){
			appendToFileCB.setEnabled(false);
		} else {
			appendToFileCB.setEnabled(true);
		}
    	...
    	val = Preferences.get("ActivityMonitoring.AlwaysStartMonitoring", null);
    	if(val instanceof Boolean ){
    		alwaysMonitorCB.setSelected(((Boolean) val).booleanValue());
    	}
