...
public class ShortcutPanel extends JPanel implements ActionListener {
    ...
    public ShortcutPanel() {
        ...
        keyStrokeClashMap = new HashMap< KeyStroke, HashMap<String, List<String>>>();
    ...
    private void loadTableModel(DefaultTableModel model, String modeName){     
    	...
    	Iterator it = allShortCuts.keySet().iterator();
    	while (it.hasNext())
    	{     		
     		allActions.add((String) it.next());     		
     	}
    ...
    private void convertToKeyBoardClashMap(String modeName){    	
    	...
    	Map.Entry pair;
    	KeyStroke ks;
    	Iterator it = shortcutKeyStrokesMap.get(modeName).entrySet().iterator();
    	while (it.hasNext()){
    		pair = (Entry) it.next();
    	...
    	if(map.size() > 0){
    		...
    		while( it.hasNext()){
    			pair = (Entry) it.next();
    			ks = (KeyStroke) pair.getKey();
    			actionList = (List<String>) pair.getValue();
    			if(actionList != null){
    ...
    private void computeClashes(){
    	...
    	Iterator it = keyStrokeClashMap.entrySet().iterator();
    	Map.Entry pair;
    	KeyStroke ks;
    	...
    	while (it.hasNext()){
    		pair = (Entry) it.next();
    		ks = (KeyStroke) pair.getKey();
    		actionMap = (HashMap<String, List<String>>) pair.getValue();    		
    		if(actionMap != null){
    			if(actionMap.size() == 1){
    				Iterator itMap = actionMap.entrySet().iterator();
        			Map.Entry pair1;
    				while(itMap.hasNext()){
    					pair1 = (Entry) itMap.next();
    					modeName = (String) pair1.getKey();
    					actionList = (List<String>) pair1.getValue();
    ...
    private boolean isKeystrokeUsed(String action, KeyStroke ks, String tabName){
		...
		if(replaceShortcutsMap.size() > 0){
			...
			Iterator it = replaceShortcutsMap.entrySet().iterator();
			while(it.hasNext()){
    ...
    private void updateClashesFor(KeyStroke ks, String modeName, String action){
    	if(keyStrokeClashMap.containsKey(ks)){    		
    		HashMap<String, List<String>> actionMap = keyStrokeClashMap.get(ks);        		
