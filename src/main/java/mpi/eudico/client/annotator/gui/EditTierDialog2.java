...
public class EditTierDialog2 extends ClosableDialog implements ActionListener,
    ...
    private HashMap currentProps = new HashMap(10);
    ...
    private void fillLingTypeMenu() {
        ...
        if (mode == CHANGE) {
            ...
            if (!onlySameStereo && curStereoType >= 0) {
                if (tier.getDependentTiers().size() > 0) {

                    // if there is at least one time alignable child only add time alignable
                    TierImpl ch;
                    for (int i = 0; i < tier.getDependentTiers().size(); i++) {
                        ch = (TierImpl) tier.getDependentTiers().get(i);
        ...
        if (tierName != null) {
            TierImpl t2 = (TierImpl) transcription.getTierWithId(tierName);
    ...
    private void fillParentComboBox() {
        ...
        if ((tier != null) && (mode != ADD)) {
        	...
        	List candidateParents = getCandidateParentTiers(tier);
    ...
    private void updateLanguageComboBox() {
        ...
        if (tier != null) {
            ...
            if (l != null) {
                 ...
                 */
            	if (l == Locale.getDefault() && ((String) languageComboBox.getItemAt(1)).startsWith(l.getDisplayName())) {
    ...
    private void doAdd(String tierName, Tier parentTier, String lingType,
        String participant, String annotator, Locale locale) {
    ...
    private void doDelete() {
        if (tier != null) {
        	Vector<TierImpl> selectedTiers = new Vector<TierImpl>();   
        	...
        	if(selectedRows.length > 0) {
        		...
        		for(int row = 0; row < selectedRows.length; row++) {
        			String tierName = (String) model.getValueAt(selectedRows[row], column);
        			TierImpl selectedTier = (TierImpl) transcription.getTierWithId(tierName);
        	...
        	Iterator selIt = selectedTiers.iterator();
        	while(selIt.hasNext()) {
        		TierImpl selT = (TierImpl) selIt.next();
        		mesBuf.append(selT.getName() + "\n");
        		Vector depTiers = selT.getDependentTiers();
...
//        	}
    ...
    private void showAdvancedOptionsDialog() {
        ...
        /* Mod by Mark */
        Object highlightColors = Preferences.get("TierHighlightColors", transcription);
        ...
        if (mode == ADD) {
        	...
        	if (parentComboBox.getSelectedItem() != none) {
        		...
        		if (colors instanceof Map) {
        			Object col = ((Map) colors).get(pName);
                ...
                if (highlightColors instanceof Map) {
                    Object col = ((Map) highlightColors).get(pName);
    		...
    		if (colors instanceof Map) {
    			Object col = ((Map) colors).get(currentTiersComboBox.getSelectedItem());
    			if (col instanceof Color) {
            		currentProps.put("TierColor", col);
    			} else {
    				String pName = (String) parentComboBox.getSelectedItem();
    				Object colP = ((Map) colors).get(pName);
            ...
            /* Mod by Mark */
            if (highlightColors instanceof Map) {
                Object col = ((Map) highlightColors).get(currentTiersComboBox.getSelectedItem());

                if (col instanceof Color) {
                    currentProps.put("TierHighlightColor", col);
                } else {
                    String pName = (String) parentComboBox.getSelectedItem();
                    Object colP = ((Map) highlightColors).get(pName);
    		...
    		if (fonts instanceof Map) {
    			Object fo = ((Map) fonts).get(currentTiersComboBox.getSelectedItem());
    ...
    private void applyAttributeSettings(){   
		...
		Color nextHighlightColor = (Color) highlightColors.get("TierHighlightColor");       
		Font fo = (Font) fonts.get("TierFont");
		...
		if (nextColor != null && !nextColor.equals(Color.WHITE) ) {
			((Map) colors).put(tierName, nextColor); 
    	}
        
        if (nextHighlightColor != null && !nextHighlightColor.equals(Color.WHITE)) {
			((Map) highlightColors).put(tierName, nextHighlightColor);
        }
        
        if (fo != null) {
			((Map) fonts).put(tierName, fo);
    	}
		...
		for(int i=0; i< tierList.size(); i++){
			...
			if (applyColor && nextColor != null && !nextColor.equals(Color.WHITE) ) {
				((Map) colors).put(t.getName(), nextColor); 
        	}else {
        		((Map) colors).remove(t.getName());
        	}
			
			if (applyHighlight && (nextHighlightColor != null) && !nextHighlightColor.equals(Color.WHITE)) {
				((Map) highlightColors).put(t.getName(), nextHighlightColor);
            } else {
            	((Map) highlightColors).remove(t.getName());
            }
		
			if (applyFont && fo != null) {
				((Map) fonts).put(t.getName(), fo);
        	} else {
        		((Map) fonts).remove(t.getName());
        	}
		}
    ...
    private void checkGraphics(TierImpl ti) {
        ...
        Iterator tierIt = tiers2.iterator();
        TierImpl t;
...
//                }
                ...
                switch (mode) {
                case ADD:
                    doAdd(tierName, parentTier, lingType, participant, annotator, locale);

                    break;

                case CHANGE:
                    doChange(tierName, parentTier, lingType, participant, annotator, locale);
