...
public class ExportResultTableAsEAF implements ClientLogger {
        ...
        new Thread() {
            public void run() {
		        ...
		        if (anc instanceof Annotation) {
		            ann = (Annotation) anc;
		            transcription = (TranscriptionImpl) ann.getTier().getParent();
		        }
		        ...
		        try {
		            store.storeTranscription(nextTrans, null, new Vector(), fileName,
     ...
     */
    private void extractSubMatches(ElanMatch match, ArrayList topAnnos, ArrayList topTiers) {
        ElanMatch chMatch;
        Annotation ann;
     ...
     */
    private void copyAnnotations(TranscriptionImpl transcription, TranscriptionImpl nextTrans, 
            ArrayList topAnnos) {
