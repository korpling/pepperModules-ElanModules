...
public abstract class AbstractMultiFileExportSaveSettingsStepPane extends StepPane implements ActionListener {	
    ...
    protected void loadPreferences() {
		...
		if(saveWithOriginalNames != null){
			prefValue = Preferences.get(saveWithOriginalNames, null);
			if (prefValue != null && prefValue instanceof Boolean) {
				originalFileNameRB.setSelected((Boolean)prefValue);
				addSuffixRB.setSelected(!(Boolean)prefValue);
			}
		}	
		
		if(saveInOriginalFolder != null){
			prefValue = Preferences.get(saveInOriginalFolder, null);
			if (prefValue instanceof Boolean) {
				boolean origFolder = (Boolean) prefValue;
				if (!origFolder) {
					prefValue = Preferences.get(saveInRelativeFolder, null);
					if (prefValue instanceof Boolean) {
		...
		if(saveInRelativeFolderName != null){
			prefValue = Preferences.get(saveInRelativeFolderName, null);
		...
		if(saveInSameFolderName != null){
			prefValue = Preferences.get(saveInSameFolderName, null);
			if (prefValue instanceof String) {
		...
		if(dontCreateEmptyFiles != null){
			prefValue = Preferences.get(dontCreateEmptyFiles, null);
			if (prefValue instanceof Boolean) {
