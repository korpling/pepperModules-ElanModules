...
public class ExportTiersDialog extends AbstractExtTierExportDialog{	
     ...
     */
    public ExportTiersDialog(Frame parent, boolean modal, ArrayList<File> files) {
	...
	private void loadPreferences() {
		...
		prefValue = Preferences.get("ExportTiersDialog.rootTiersCB", null);
		if (prefValue instanceof Boolean) {
			setRootTiersOnly((Boolean) prefValue);
		}
		prefValue = Preferences.get("ExportTiersDialog.SelectTiersMode", null);
		if (prefValue instanceof String) {
			setSelectionMode((String) prefValue);
		}
		
		prefValue = Preferences.get("ExportTiersDialog.forceParentExport", null);
		if (prefValue instanceof Boolean) {
			boolean forceParent = (Boolean) prefValue;
			if (!forceParent) {
				neglectDependentTiersRB.setSelected(true);
			}
		}
		prefValue = Preferences.get("ExportTiersDialog.saveWithOriginalNames", null);
		if (prefValue instanceof Boolean) {
			boolean origNames = (Boolean) prefValue;
			if (!origNames) {
				addSuffixRB.setSelected(true);
				prefValue = Preferences.get("ExportTiersDialog.saveWithSuffix", null);
				if (prefValue instanceof Boolean) {
					boolean withSuffix = (Boolean) prefValue;
					if (!withSuffix) {
		...
		prefValue = Preferences.get("ExportTiersDialog.fileNameSuffix", null);
		if (prefValue instanceof String) {
			suffixNameTextField.setText((String) prefValue);
		}
		prefValue = Preferences.get("ExportTiersDialog.fileBaseName", null);
		if (prefValue instanceof String) {
			baseNameTextField.setText((String) prefValue);
		}
		prefValue = Preferences.get("ExportTiersDialog.saveInOriginalFolder", null);
		if (prefValue instanceof Boolean) {
			boolean origFolder = (Boolean) prefValue;
			if (!origFolder) {
				prefValue = Preferences.get("ExportTiersDialog.saveInRelativeFolder", null);
				if (prefValue instanceof Boolean) {
		...
		prefValue = Preferences.get("ExportTiersDialog.saveInRelativeFolderName", null);
		if (prefValue instanceof String) {
			localDirectoryTextField.setText((String) prefValue);
		}
		prefValue = Preferences.get("ExportTiersDialog.saveInSameFolderName", null);
		if (prefValue instanceof String) {
			sameDirectoryTextField.setText((String) prefValue);
		}
		prefValue = Preferences.get("ExportTiersDialog.dontCreateEmptyFiles", null);
		if (prefValue instanceof Boolean) {
			dontExportFilesWithoutTiersCB.setSelected((Boolean) prefValue);
		}
	...
	private class SaveFileThread extends Thread{
		...
		public SaveFileThread( List<String> tiersToSave ){
			this.tiersToSave = (List<String>) tiersToSave;	
		}
			...
			for( int i=0; i<tiers.size(); i++ ){
				...
				if( !exists( tier.getName(), tiersToSave ) ){ 
					...
					if( neglectDependentTiersRB.isSelected() ){						
						...
						for( int n=0; n<dependentTiers.size(); n++ )
							if( exists( ((TierImpl)dependentTiers.get(n)).getName(), tiersToSave ) ){
		        	 ...
		        	 if( directoryExists )
		        		 ...
		        		 if( saveForever || saveThisFile ){
		        			 if( !dontExportFilesWithoutTiersCB.isSelected() )
		        				 ACMTranscriptionStore.getCurrentTranscriptionStore().storeTranscriptionIn(transImpl, null, new Vector(), directoryToSave, TranscriptionStore.EAF);
		        			 else{
