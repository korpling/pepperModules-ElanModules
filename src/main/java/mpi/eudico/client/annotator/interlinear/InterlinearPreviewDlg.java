...
//        getContentPane().add(optionsPanel, gbc);
    ...
    protected void extractTiers() {
        if (model != null) {
            ...
            if (interlinearizer.getTranscription() != null) {            	
            	Vector v ;
            	TierImpl t;
            	List tierOrder = mpi.eudico.client.annotator.commands.ELANCommandFactory.getViewerManager(interlinearizer.getTranscription()).getTierOrder().getTierOrder();
            	if(tierOrder != null && tierOrder.size() > 0){
            		v = new Vector();
            		for (int i = 0; i < tierOrder.size(); i++) {
            			t = (TierImpl) interlinearizer.getTranscription().getTierWithId(tierOrder.get(i).toString());
            			if(t != null){
            				v.add(t);
            			}
            		}
    ...
    protected List getCurrentTierOrder() {  
        ...
        for (int i = 0; i < model.getRowCount(); i++) { 
        	tierOrder.add(model.getValueAt(i, tierCol));            
        }
     ...
     */
    private void selectTiers(List visibleTiers) { 
    ...
    private void doApplyChanges() {
        ...
        if (interlinearizer.isPlaySoundSel()){
     ...
     // check the minimal silence duration
        int minSilence = interlinearizer.MIN_SILENCE;
...
//        applyFontSizes();
    ...
    public void actionPerformed(ActionEvent event) {
        if (event.getSource() == applyChangesButton) {
        	...
        	Vector selectedTiers = new Vector(getSelectedTierList());       	
        	...
        	// read prefs
        	List hiddenTiers = (List) Preferences.get(interlinearizer.prefHiddenTiers, interlinearizer.getTranscription());
        	String selectionMode = (String) Preferences.get(interlinearizer.prefTierSelectionMode, interlinearizer.getTranscription());
        	
        	if(hiddenTiers != null){
        		int i=0;
        		TierImpl tier;
        		while(i < hiddenTiers.size()){
        			tier = (TierImpl) interlinearizer.getTranscription().getTierWithId((String) hiddenTiers.get(i));
        ...
        if (ie.getSource() == sortingComboBox) {
            ...
            if (silenceCB.isSelected()) {
                ...
                if (minDurSilTF.getText() == null || minDurSilTF.getText().length() == 0) {
                    minDurSilTF.setText("" + interlinearizer.MIN_SILENCE);
                }
