...
/**
 * DOCUMENT ME! $Id: ElanFrame2.java 34665 2012-12-06 12:55:12Z hasloe $
 *
 * @author $Author$
 * @version $Revision$
 */
        ...
        try {
            ...
            for (int i = 0; i < mediaDescriptors.size(); i++) {
                ...
                if (!file.exists()) {
                    ...
                    chooser.createAndShowFileDialog(ElanLocale.getString("Frame.ElanFrame.LocateMedia") + ": " + localFileName, FileChooser.OPEN_DIALOG, null, 
...
//        	}
    ...
    private void initMenuBar() {
       ...
       // JMenuItem menuItemMultipleFileAnnotationsFromOverlaps = new JMenuItem(ma);
        ...
        ma = new ImportToolboxMultiMA(ELANCommandFactory.IMPORT_TOOLBOX, this);
        menuActions.put(ELANCommandFactory.IMPORT_TOOLBOX, ma);
        importMenuMulti.add(new JMenuItem(ma));      
        
        ma = new ImportPraatMultiMA(ELANCommandFactory.IMPORT_PRAAT_GRID, this);
        menuActions.put(ELANCommandFactory.IMPORT_PRAAT_GRID, ma);
        importMenuMulti.add(new JMenuItem(ma));      
...
//        menuViewer.add(menuItemInterLinearizerViewer);
        ...
        // temporarily include an option for web services
//        ma = new MenuAction(ELANCommandFactory.WEBSERVICES_DLG);
//        webServicesMI = new ElanMenuItem(ElanLocale.getString(ELANCommandFactory.WEBSERVICES_DLG), false);
//        menuActions.put(ELANCommandFactory.WEBSERVICES_DLG, ma);
//        menuOptions.add(webServicesMI);
//        menuOptions.addSeparator();
...
//        menuOptions.add(menuItemRateVol);
    ...
    public void clearShortcutsMap(String modeConstant){
    	...
    	Iterator it = ShortcutsUtil.getInstance().getCurrentShortcuts(modeConstant).entrySet().iterator();
        while (it.hasNext())
        {
        	Map.Entry pairs = (Map.Entry) it.next();
        	String actionName = (String) pairs.getKey();        	
        	...
        	if(ca != null){
            	...
            	String ksDescription = ShortcutsUtil.getInstance().getDescriptionForKeyStroke((KeyStroke) pairs.getValue());
...
//    	}
        ...
        if (inputMap instanceof ComponentInputMap && (actionMap != null)) {
            ...
            Iterator it = shortMap.entrySet().iterator();
            ...
            while (it.hasNext())
            {
            	...
            	if(act != null){        				
            		...
            		if(ksDescription != null && ksDescription.trim().length() > 0){
                		String shortValue = (String) act.getValue(act.SHORT_DESCRIPTION);
                		if(shortValue != null){
...
//        	}        	
//        }  
//        menuActions.remove(ELANCommandFactory.WEBSERVICES_DLG);
//        webServicesMI.setAction(ELANCommandFactory.getCommandAction(
//        		transcriptionForThisFrame, ELANCommandFactory.WEBSERVICES_DLG));
//        webServicesMI.setEnabled(true);
//        menuOptions.add(menuItemAnnoMode);
//        menuOptions.add(menuItemSyncMode);
        //menuOptions.addSeparator();
...
//        }
        ...
        Iterator it = menuActions.values().iterator();
		while(it.hasNext()){
			ma = (MenuAction) it.next();
			if (ma !=null) {				
	        	ma.updateLocale();
			}
		}
...
//    	}
    ...
    private void loadCVPreferences() {
    	...
    	Object cvPrefs = Preferences.get("CV.Prefs", transcriptionForThisFrame);
    	if (cvPrefs instanceof HashMap) {
    		HashMap cvPrefMap = (HashMap) cvPrefs;
    		List allCV = ((TranscriptionImpl) transcriptionForThisFrame).getControlledVocabularies();
    		if (allCV == null) {
    			return;
    		}
			String color = "Color";
    		...
    		for (int i = 0; i < allCV.size(); i++) {
    			cv = (ControlledVocabulary) allCV.get(i);
    			...
    			if (cvMapObj instanceof HashMap) {
    				...
    				for (int j = 0; j < cve.length; j++) {
    					if (cvMap.containsKey(cve[j].getValue())) {
    						cveMap = (HashMap) cvMap.get(cve[j].getValue());
    						if (cveMap.containsKey(color)) {
    							cve[j].setPrefColor((Color)cveMap.get(color));
    						}
    						if (cveMap.containsKey(keyCode)) {
    							cve[j].setShortcutKeyCode(((Integer)cveMap.get(keyCode)).intValue());
    						}
    					}
    				}
    			}
