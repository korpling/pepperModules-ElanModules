	...
	public void setLayoutManager(ElanLayoutManager layoutManager) {
		...
		if (layoutManager != null) {
			...
			if (val instanceof Long) {
				duration = ((Long) val).longValue();
			}
			val = Preferences.get("ImagePlayer.CursorVisible", layoutManager.getViewerManager().getTranscription());
			if (val instanceof Boolean) {
				cursorVisible = ((Boolean) val).booleanValue();
				if (visComponent != null) {
					visComponent.setCursorVisible(cursorVisible);
				}
			}
			val = Preferences.get("ImagePlayer.CursorGrid.Columns", layoutManager.getViewerManager().getTranscription());
			if (val instanceof Integer) {
				cursorGrid[0] = ((Integer) val).intValue();
			}
			val = Preferences.get("ImagePlayer.CursorGrid.Rows", layoutManager.getViewerManager().getTranscription());
			if (val instanceof Integer) {
				cursorGrid[1] = ((Integer) val).intValue();
			}
			if (visComponent != null) {
				visComponent.setCursorGrid(cursorGrid[0], cursorGrid[1]);
			}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("detach")) {
			...
			if (visComponent != null && cursorVisItem != null) {
	        	...
	        	layoutManager.setPreference("ImagePlayer.CursorVisible", new Boolean(visComponent.isCursorVisible()), 
	        			layoutManager.getViewerManager().getTranscription());
			}
			...
			if (option == JOptionPane.OK_OPTION) {
	        	...
	        	layoutManager.setPreference("ImagePlayer.CursorGrid.Columns", new Integer(cursorGrid[0]), 
	        			layoutManager.getViewerManager().getTranscription());
	        	layoutManager.setPreference("ImagePlayer.CursorGrid.Rows", new Integer(cursorGrid[1]), 
	        			layoutManager.getViewerManager().getTranscription());
			}
		}
