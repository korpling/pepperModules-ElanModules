...
public class TranscriptionModePlayerController implements ChangeListener, ActionListener {
    ...
    public JPanel getVolumeRatePanel() {
    	...
    	int volume = (int)(vm.getMasterMediaPlayer().getVolume() * 100);    	
    	Float value = (Float) Preferences.get("MediaControlVolume", vm.getTranscription());
    	if(value != null){
    	...
    	value = (Float) Preferences.get("MediaControlRate", vm.getTranscription());
    	if(value != null){
    ...
    public JPanel getModePanel(){
    	...
    	loopModeCB = new JCheckBox(ElanLocale.getString(ELANCommandFactory.LOOP_MODE));
 	    loopModeCB.setFont(Constants.SMALLFONT);
    ...
    public void stateChanged(ChangeEvent e) {
		if (e.getSource() == volSlider) {
			...
			vm.getMasterMediaPlayer().setVolume(((float) volume / 100));			
		} 
