...
public class TranscriptionTable extends JTable {
	...
	public TranscriptionTable( ) {	
		...
		prefTierFontColors = new HashMap();
...
//				   	}				   	
	...
	public void goToEditableCellUp(){
		int rowBefore = getCurrentRow() -1;
   		...
   		while(true){
   			...
   			if (getValueAt(rowBefore,getCurrentColumn()) instanceof String && !autoCreateAnn ){
   				if(rowBefore == 0){
   					rowBefore = getCurrentRow();
   					break;
   				} else {
	...
	public void goToEditableCellDown(){
		int rowNext = getCurrentRow() + 1;
   		...
   		while(true){
   			...
   			if (getValueAt(rowNext,getCurrentColumn()) instanceof String && !autoCreateAnn ){
   				if(rowNext >= getRowCount()){
   					rowNext = getCurrentRow();
   					break;
   				} else {
	...
	public void goToEditableCellLeft(){
		...
		while(true){
			...
			if (getValueAt(getCurrentRow(),leftColumn) instanceof String && !autoCreateAnn ){
				leftColumn = leftColumn - 1;
				continue;
	   		}
	...
	public void goToEditableCellRight(){
		...
		while(true){
			...
			if (getValueAt(getCurrentRow(),rightColumn) instanceof String  && !autoCreateAnn ){
				rightColumn = rightColumn + 1;
				continue;
			}
	...
	public void goToNextEditableCell(){
   		if(moveViaColumn){   			
   			...
   			while(true){
   				...
   				if (getValueAt(nextRow,nextColumn) instanceof String && !autoCreateAnn ){
   					nextColumn = nextColumn+1;
   					continue;
   				}
...
//		   	}
	...
	public void setFontColorForTiers(HashMap fontColors) {
		...
		if (fontColors != null) {
			...
			Object key;
			Object val;
			Iterator keyIt = fontColors.keySet().iterator();
			while (keyIt.hasNext()) {
				key = keyIt.next();
				val = fontColors.get(key);
				if (val instanceof Color) {
					prefTierBrighterFontColors.put((String) key, brighter((Color) val));
					prefTierBrightestFontColors.put((String) key, brightest((Color) val));
				}
			}
		}		
	}
	...
	public Color getFontColorForTier(String tierName) {
		return (Color) prefTierFontColors.get(tierName);
	}
	...
	public Color getBrighterFontColorForTier(String tierName) {
		return (Color) prefTierBrighterFontColors.get(tierName);
	}
	...
	public Color getBrightestFontColorForTier(String tierName) {
		return (Color) prefTierBrightestFontColors.get(tierName);
	}
	...
	public void setFontsForTiers(HashMap fontNames) {
		...
		if (fontNames != null) {
			...
			while (keyIt.hasNext()) {
				...
				fn = ((Font) fontNames.get(key)).getName();
				if (fn != null) {
	...
	public Font getFontForTier(String tierName) {
		return (Font) prefTierFonts.get(tierName);
	}
