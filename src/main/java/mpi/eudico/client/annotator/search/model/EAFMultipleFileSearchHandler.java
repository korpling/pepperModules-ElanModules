...
class EAFMultipleFileSearchHandler extends DefaultHandler {
    ...
    final private Hashtable timeSlots;

    final private Hashtable timeUnsolvedMatches;

    final private Hashtable ref1;

    final private Hashtable ref2;

    final private List tierNames;
    ...
    public EAFMultipleFileSearchHandler(ContentQuery query) {
        ...
        ref1 = new Hashtable();
        ref2 = new Hashtable();
        timeUnsolvedMatches = new Hashtable();
		unalignedAlignablesIds = new ArrayList();
		unalignedAlignableMatches = new ArrayList();
    }
    ...
    public void endElement(String namespaceURI, String sName, String qName)
        ...
        if (qName.equals("ANNOTATION_VALUE")) {
            ...
            if (matcher.find()) {
                ...
                match.setMatchedSubstringIndices((int[][]) substringIndices
                        .toArray(new int[0][0]));
                ...
                // do something for ref annotations,
                    ...
                    while (ref1.get(alignedAnnID) == NULL) {
                        ...
                        alignedAnnID = (String) ref2.get(alignedAnnID);
                    }
			...
			if (lastAlignedBTS != null) {
				if (timeSlots.containsKey(timeSlotRef2)) {
					...
					//unalignedAlignablesIds.add(new Integer(indexInTier));
    ...
    public void endDocument() {
        try {
            Enumeration e = timeUnsolvedMatches.keys();
            while (e.hasMoreElements()) {
                EAFMultipleFileMatch match = (EAFMultipleFileMatch) e.nextElement();
                String alignedAnnID = (String) timeUnsolvedMatches.get(match);
                while (ref1.get(alignedAnnID) == NULL) {
                    // are there more than 1 refs possible?
                    alignedAnnID = (String) ref2.get(alignedAnnID);
                }
    ...
    private void setTimeForMatch(EAFMultipleFileMatch match, Object timeSlotRef1,
        ...
        if (timeSlotRef1 != null) {
            String value = (String) timeSlots.get(timeSlotRef1);
            if (value != null) {
                match.setBeginTimeBoundary(Long.valueOf(value).longValue());
            }
        }
        if (timeSlotRef2 != null) {
            String value = (String) timeSlots.get(timeSlotRef2);
            if (value != null) {
    ...
    private void calculateUnalignedAlignedMatches() {
    	if (unalignedAlignablesIds.size() != 0) {
    		...
    		try {
    			refBT = Long.valueOf((String)timeSlots.get(lastAlignedBTS)).longValue();
    		} catch (NumberFormatException ex) {   			
    		}
    		
			try {
				refET = Long.valueOf((String)timeSlots.get(timeSlotRef2)).longValue();
			} catch (NumberFormatException ex) {		
			}
			...
			for (int i = 0; i < unalignedAlignablesIds.size(); i++) {
				String id = (String) unalignedAlignablesIds.get(i);
				String tsRef2 = (String) ref2.get(id);
			...
			for (int i = 0; i < unalignedAlignableMatches.size(); i++) {
				EAFMultipleFileMatch match = (EAFMultipleFileMatch) unalignedAlignableMatches.get(i);
				String id = match.getId(); 
				
				if (match.getBeginTimeBoundary() == -1) {
					String tsRef1 = (String) ref1.get(id);
					String value = (String) timeSlots.get(tsRef1);
					if (value != null) {
						match.setBeginTimeBoundary(Long.valueOf(value).longValue());
					}
				}
				if (match.getEndTimeBoundary() == -1) {
					String tsRef2 = (String) ref2.get(id);
					String value = (String) timeSlots.get(tsRef2);
					if (value != null) {
						match.setEndTimeBoundary(Long.valueOf(value).longValue());
					}
				}
